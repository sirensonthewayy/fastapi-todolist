...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:3000"], # В продакшене лучше указать конкретные домены
...
def create_todo_item(item: schemas.ToDoItemCreate, db: Session =
Depends(get_db)):
...
def read_todo_items(skip: int = 0, limit: int = 10, db: Session =
Depends(get_db)):
    items = db.query(models.ToDoItem).offset(skip).limit(limit).all()
    return items

@app.put("/todoitems/{item_id}", response_model=schemas.ToDoItem)
def update_todo_item(item_id: int, item: schemas.ToDoItemCreate, db: Session =
Depends(get_db)):
...
def delete_todo_item(item_id: int, db: Session = Depends(get_db)):
    db_item = db.query(models.ToDoItem).filter(models.ToDoItem.id ==
    item_id).first()
    if db_item is None:
        raise HTTPException(status_code=404, detail="Item not found")
    db.delete(db_item)
