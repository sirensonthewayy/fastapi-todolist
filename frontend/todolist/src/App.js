...
const App = () => {
	...
	const fetchData = async () => {
		try {
			const response = await axios.get('http://127.0.0.1:8001/todoitems/');
	...
	const addTodoItem = async () => {
		...
		try {
			const response = await axios.post('http://127.0.0.1:8001/todoitems/',
				newItem);
	...
	const updateTodoItem = async (id) => {
		...
		try {
			const response = await
				axios.put(`http://127.0.0.1:8001/todoitems/${id}`, updatedItem);
			setTodoItems(todoItems.map(item => (item.id === id ? response.data :
				item)));
			setEditItem(null);
		} catch (error) {
	...
	const deleteTodoItem = async (id) => {
		try {
			await axios.delete(`http://127.0.0.1:8001/todoitems/${id}`);
	...
	return (
		<div>
			...
			<Container>
				<Typography variant="h4" style={{ marginTop: '20px' }}>Create a
					new task</Typography>
				<TextField
					label="Title"
					value={newTitle}
...
<Delete />
			...
			<Dialog open={Boolean(editItem)} onClose={closeEditDialog}>
				...
				<DialogActions>
					...
					<Button onClick={() => updateTodoItem(editItem.id)}
						color="primary">
						Save
					</Button>
				</DialogActions>
